...
Status: accepted for S023; extended by S024 TextVisual and S025 MeshVisual boundaries.
...
- create retained visuals with family constructors such as `dvz_point`, `dvz_marker`, `dvz_segment`,
  `dvz_path`, `dvz_image`, and capability-gated `dvz_mesh`;
...
## S025 MeshVisual boundary

Datoviz v0.4 mesh support must be proven against retained v0.4 APIs before implementation. Evidence
may include `dvz_mesh`, indexed geometry upload, dense `position`/`color`/`normal`/`texcoords` slots,
material/depth helpers, and query APIs, but those names remain backend implementation details. The
GSP public protocol exposes only accepted `MeshVisual` semantics.

Strict S025 Datoviz support requires exact flat rendering of indexed 2D/3D triangle geometry with
uniform or per-face RGBA. Exact 2D-to-3D `z=0` adaptation is allowed when documented. Hidden lighting,
