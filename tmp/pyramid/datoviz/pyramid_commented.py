...
dtype: type = np.float16  # Data type for memory-efficient storage
vmin: float = -5e-4  # Minimum value for data normalization
vmax: float = +5e-4  # Maximum value for data normalization
tex_size: int = 2048  # Maximum texture size in pixels (GPU texture limit)
max_res: int = 11  # Maximum resolution level in the pyramid (2^11 = 2048x downsampling)
...
def path(i: int) -> Path:
    ...
    Args:
        i: Resolution level (0 = full resolution, higher = more downsampled)
...
def load_file(res: int) -> Optional[np.memmap]:
    ...
    if not fp.exists():
        return
    size = fp.stat().st_size  # Get file size in bytes
    assert size % (n_channels * 2) == 0  # Verify file size is consistent (2 bytes per float16)
    n_samples = int(round(size / (n_channels * 2)))  # Calculate number of time samples
    out = np.memmap(fp, shape=(n_samples, n_channels), dtype=dtype, mode="r")
    print(f"Memmap file with shape {out.shape}")
    return out


def safe_slice(data: npt.NDArray, i0: int, i1: int, fill_value: float = 0) -> npt.NDArray:
...
def on_frame(ev: dvz.Event) -> None:
    ...
    for new_res in range(0, max_res + 1):
        ...
        if i1 - i0 <= tex_size:  # Data fits in texture at this resolution
            break
