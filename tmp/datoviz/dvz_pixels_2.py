...
point_count = 10000
position_new = np.random.rand(point_count, 3).astype(np.float32) * 2.0 - 1
position_new *= 0.9
color_new = np.array([[255, 0, 0, 255] * point_count], dtype=np.uint8).reshape(point_count, 4)

# visual = app.pixel(position=position, color=color)
visual = app.pixel(position=position_new, color=color_new)
panel.add(visual)
