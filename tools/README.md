...
- `run_datoviz_texture2d_checkpoint.sh`: focused S050 nearest-only Texture2D checkpoint. It verifies exact sibling-checkout provenance and current field-slot sampling symbols, renders the five isolated Texture2D cases, validates every Datoviz artifact, runs the current-protocol VisPy2 example, and executes focused adapter/producer tests.
