...
4. Regenerate the tracked ctypes binding with the normal Datoviz command.
5. Extend generator/policy/smoke tests so both state records must have the complete required fields, positive size, correct offsets, and ABI size/alignment. At minimum verify the ABI prologue, state fields, camera/projection fields, orthographic bounds, and matrices.
6. Add a bounded native readback smoke or equivalent existing focused test proving both output records are safe to pass to their public C functions and that returned `struct_size` equals the generated ctypes size.
7. Keep the public C structs and functions unchanged unless evidence proves the declared ABI itself is wrong. Do not modify GSP, VisPy2, the `data` submodule, paper files, runtime libraries, release metadata, or unrelated Datoviz work.
...
- No generated state record remains zero-size.
- Python and C ABI size, alignment, and field offsets agree.
