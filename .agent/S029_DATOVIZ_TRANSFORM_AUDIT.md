...
linear `View2D` DATA mapping. M127 refreshed this evidence for Datoviz native panel-domain
mapping after `DvzPanelView2D` became policy-only.
...
| `transform/view2d_data_ndc_overlay` | `strict` | Finite eager point DATA positions rendered through native ordered panel domains, including reversed x limits, aligned with explicit VIEW overlay points. |
| `transform/family_affine_view2d` | `strict` | One required named AFFINE_2D resource applied before native DATA-domain rendering for point, marker, segment, path, center-anchored text, and strict 2D uniform mesh visuals. |
...
- `src/gsp_datoviz/protocol_renderer.py` sets ordered panel domains through
  `dvz_panel_set_domain()` before enabling policy-only `DvzPanelView2D`.
- `tests/test_datoviz_v04_protocol_renderer.py` covers inline affine adaptation, named transform
  resolution, missing required resource rejection, and reversed-limit native `View2D` DATA domains.
- Current contact sheets show Datoviz matching Matplotlib for inline/named point equivalence,
  reversed `View2D` DATA-to-VIEW overlay alignment, and cross-family affine plus `View2D` rendering.
