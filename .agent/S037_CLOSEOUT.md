...
| Datoviz v0.4 | static View3D mesh rendering supported with local P022 bindings | Uses panel-owned camera state plus explicit orthographic bounds. Live View3D navigation and GPU 3D visual picking remain deferred. |
...
Claim Datoviz static public `View3D` mesh rendering only for local `v0.4-dev` builds with the P022
camera bindings. The next actionable work is either:

- M167 Datoviz `query.view3d.ray_readback.v1` parity for canonical ray-context payloads; or
