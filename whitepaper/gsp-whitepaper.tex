...
browser-based charting system, or a low-level GPU library. \matplotlib established a durable model
for publication-oriented plotting in Python \cite{Hunter:2007}; VTK demonstrated the value of a
large scientific visualization toolkit for geometry, imaging, and 3D pipelines
\cite{Schroeder:2006}; Vega-Lite showed how far a declarative visualization grammar can go in the
browser \cite{Satyanarayan:2017}. These projects differ in user experience, language ecosystem,
deployment target, and visual style. That diversity is valuable: no single user API can cover the
needs of static publication plots, interactive exploratory tools, microscopy viewers,
computational geometry, neural data inspection, volume rendering, and web dashboards.
...
They need text rendering, antialiasing, color mapping, transparency, and image resampling to be
good enough for scientific inspection, not merely sufficient for a demo
\cite{Parker:1983,Mitchell:1988,Bujack:2018,McGuire:2013,Rougier:2018}. They need to run locally, in
notebooks, in CI, on remote machines, and eventually in browser or cloud rendering contexts.
...
Most scientific figures decompose into a limited number of visual components. A scatter plot, for
example, is not a magical object from the renderer's perspective. It is a composition of points or
markers, line segments, text glyphs, axes, and filled regions. A high-level plotting API may choose
how much of that structure to expose, but the renderer still has to draw the underlying
components.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{plot-decomposition.pdf}
  \caption{A scatter plot decomposed into elementary visual components. \gsp treats these
  components as semantic visual families rather than as backend draw calls.}
  \label{fig:decomposition}
\end{figure}

Primitive decomposition is necessary but not sufficient. A protocol cannot simply say ``draw
points'' or ``draw text'' and be useful to scientific visualization libraries. It also has to
define what coordinate space the points live in, what units the marker size uses, how scalar values
become colors, what identity a rendered item carries, how the view transforms the data, and what
happens when a backend cannot implement the exact request.

\gsp therefore treats primitive decomposition as the beginning of the design, not the end. The
shared layer must let libraries reuse renderer work without rebuilding a complete renderer engine,
while adding three constraints:
...
inspired by successful protocol layers such as the Language Server Protocol \cite{Microsoft:LSP}:
the important idea is not a particular encoding, but a durable boundary between producers and
implementations.

A minimal session has a concrete shape:

\begin{enumerate}
  \item the producer opens a session and states the protocol versions it can speak;
  \item the renderer returns a capability snapshot;
  \item the producer creates resources such as buffers, textures, views, and visual descriptions;
  \item the producer submits a frame or a retained scene update;
  \item the renderer returns a render result with diagnostics and snapshot identifiers;
  \item the producer may issue queries against those snapshots and receive structured readback
  payloads.
\end{enumerate}

The following sketch is intentionally not a Python API commitment. It shows the level of
abstraction: the producer describes a scene and asks a renderer to plan it against declared
capabilities.

\begin{lstlisting}[float=htbp,language=Python,caption={Illustrative producer-side session sketch.}]
session = gsp.connect("inproc://datoviz")
caps = session.initialize(protocol_versions=["gsp/0.2"])

scene = gsp.Scene(canvas=gsp.Canvas(width=900, height=600))
panel = scene.add_panel(rect=(0, 0, 900, 600), view=gsp.View2D(xlim=(0, 1), ylim=(0, 1)))
points = scene.add(
    gsp.PointVisual(
        panel=panel,
        positions=gsp.Array(points_xy, coordinate_space="data"),
        color=gsp.ScalarColor(values, scale="viridis", clim=(0, 1)),
        diameter_px=7,
    )
)

plan = gsp.plan(scene, caps)
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{gsp-architecture.pdf}
  \caption{Multiple visualization APIs can target an intermediate rendering protocol rather than
  binding directly to incompatible rendering engines. \gsp adds capability discovery, conformance,
  query/readback, and transport independence to this shared-protocol structure.}
...
Scientific visualization primitives can be described as dots, points, markers, segments, paths,
glyphs, images, meshes, and volumes. \gsp uses the term visual family to emphasize that the
protocol contract is semantic, not merely graphical. A visual family defines the meaning of data
fields, coordinate spaces, screen-space units, color interpretation, ordering rules, query
identity, and backend adaptation behavior.
...
For example, a point-family contract is not just ``draw circles.'' It contains several semantic
commitments:

\begin{center}
\begin{tabularx}{0.92\textwidth}{@{}lX@{}}
\toprule
Point field or behavior & Semantic commitment \\
\midrule
Position & Coordinates are interpreted in the visual's declared coordinate space and through the
attached view snapshot. \\
Diameter & Size is a logical screen-pixel diameter, not data-space radius and not backend marker
area. \\
Color & RGBA values are final colors unless an explicit scalar color encoding is attached. \\
Identity & Each item has stable visual and item identity for query/readback where advertised. \\
Adaptation & Unsupported coordinate spaces, color encodings, or query payloads produce diagnostics
rather than silent fallback. \\
\bottomrule
\end{tabularx}
\end{center}

A compact debug fixture for such a visual might look like this:

\begin{lstlisting}[float=htbp,caption={Illustrative debug fixture fragment for a point visual.}]
{
  "visual_family": "point",
  "visual_id": "cells",
  "panel_id": "main",
  "positions": {"resource": "xy", "dtype": "float32", "shape": [1000000, 2],
                "coordinate_space": "data"},
  "diameter_px": 5,
  "color": {"scalar_resource": "cluster_score", "scale": "viridis", "clim": [0, 1]},
  "query_payloads": ["visual_id", "item_id", "data_xy", "display_rgba"]
}
\end{lstlisting}
...
The capability surface therefore needs to be granular. A backend should be able to say ``I can
render this visual family strictly,'' ``I can render it only after CPU adaptation,'' or ``I can
render it but cannot answer strict queries.'' A planner can then make an explicit decision before
any pixels are drawn:

\begin{lstlisting}[float=htbp,caption={Illustrative capability-driven planning result.}]
request:
  visual: mesh
  coordinates: data3d
  material: flat_lambert
  query: mesh_triangle_pick

datoviz-v0.4 plan:
  render.mesh.data3d: accept
  material.flat_lambert: accept_via_cpu_resolve
  query.mesh_triangle_pick: reject
  diagnostics:
    - code: GSP_QUERY_CAPABILITY_UNSUPPORTED
      detail: native visual_id/primitive_index mapping not advertised
\end{lstlisting}

This kind of planning result is more useful than a binary backend label. It allows a producer to
continue with rendering, disable the picking tool, choose another backend, or show a precise
...
A typical query is tied to a rendered snapshot, not just to current mutable state. This prevents a
producer from interpreting a hit against a stale camera, layout, or view transform:

\begin{lstlisting}[float=htbp,caption={Illustrative panel query and response.}]
query:
  panel_id: main
  layout_snapshot_id: layout-42
  view_snapshot_id: view-107
  panel_xy_px: [514, 238]
  scope: all-rendered
...
response:
  ...
  item_id: 481516
  data_xy: [0.372, 0.811]
  display_rgba: [68, 1, 84, 255]
  payload:
    scalar_name: cluster_score
    scalar_value: 0.61
\end{lstlisting}

The exact payload varies by visual family and advertised query capability. The important property
is that the result is not merely a pixel color. It is a protocol-level explanation of a rendered
contribution.
...
\datoviz is the flagship high-performance GPU backend \cite{Rossant:Datoviz}. It brings the project back to one of its
...
artist and figure model \cite{Hunter:2007}. \gsp asks whether a protocol layer can sit below
multiple scientific visualization APIs, not only one.

Vega and Vega-Lite demonstrate the power of declarative visualization specifications, especially in
browser contexts \cite{Satyanarayan:2017}. \gsp is lower level and more renderer-oriented. It is
not a grammar for charts; it is a protocol for semantic visual scene execution, including local GPU
paths and query/readback.

VTK and similar systems provide extensive scientific visualization capabilities, especially for 3D,
meshes, and volumes \cite{Schroeder:2006}. \gsp is not a replacement for a large visualization
toolkit. It is a protocol surface that could, in principle, target or interoperate with such
renderers for suitable subsets.

ANARI is a relevant comparison for 3D scientific rendering interoperability \cite{Khronos:ANARI}.
\gsp is broader in the sense that it targets 2D and 3D scientific visualization, guides, views,
query semantics, and producer/backend separation for plotting-like systems. It is narrower in other
ways: it is not a general physically based rendering API.

Browser graphics stacks, WebGPU, WebGL, Canvas, SVG, and WebAssembly provide important execution
targets \cite{W3C:WebGPU}. \gsp should be able to target browser renderers, but it should not be
defined by browser constraints alone. Scientific desktop use still needs a fast local in-process
path.
