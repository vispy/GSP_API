...
def main():
    ...
    def generate_lines(line_count: int, point_per_line: int):
        vertex_positions = []
        vertex_colors = []
        line_widths = []
        ...
        for i in range(num_lines):
            ...
            line_width = 1 + 3.0 * np.abs(np.gradient(y))

            # Build line segments for this line
            vertex_position = [x, y, 0]

            vertex_positions.append(vertex_position)
            vertex_colors.append(color_values)
            line_widths.append(line_width)
        return vertex_positions, vertex_colors, line_widths

    num_lines = 10
    num_points = 100
    vertex_positions, vertex_colors, line_widths = generate_lines(num_lines, num_points)
