...
class AxesManaged:
    ...
    def __init__(
        ...
        viewport: Viewport,
        viewport_events: ViewportEventsBase,
        animator_base: AnimatorBase,
    ):
        self.renderer = renderer
        self.viewport = viewport
        self.viewport_events = viewport_events
        self.animator = animator_base

        # Create an AxesDisplay for the inner viewport
        self.axes_display = AxesDisplay(renderer.get_canvas(), viewport)
        # Create an AxesPanZoom to handle pan and zoom for the axes display
        self.axes_pan_zoom = AxesPanZoom(viewport_events, base_scale=1.1, axes_display=self.axes_display)
        ...
        self.axes_display.new_limits_event.subscribe(on_new_limits)

        @self.animator.event_listener
    ...
    def get_axes_display(self) -> AxesDisplay:
        return self.axes_display

    def get_axes_pan_zoom(self) -> AxesPanZoom:
        return self.axes_pan_zoom
    ...
    def remove_render_item(self, render_item: RenderItem) -> None:
        ...
        self.render_items = [ri for ri in self.render_items if ri != render_item]
    ...
    def render(self) -> None:
        ...
        render_items = self.render_items.copy()

        # Get the axes transform matrix
        axes_transform_numpy = self.axes_display.get_transform_matrix_numpy()
        ...
        render_items_axes = self.axes_display.get_render_items()
...
def main():
    ...
    # axes_pan_zoom.set_pan_limits_dunit(-15.0, +15.0, -15.0, +15.0)
