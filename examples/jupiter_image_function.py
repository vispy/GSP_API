...
from gsp.types import BufferType, CapStyle, JoinStyle
...
def generate_jupiter_image(period: float, canvas_width: int, canvas_height: int) -> bytes:
    ...
    def generate_lines(line_count: int, points_per_line: int):
        ...
        for line_index in range(line_count):
            ...
            y = np.sin(x * np.pi * 2.0 * period + line_index * 0.3) / line_count + line_index * 1.4 / line_count  # vertical offset for each line
            y -= 0.7
            z = np.zeros_like(x)
...
def main():
    """Main function for the Jupiter image example."""
    # Generate the image
    canvas_width = 400
    canvas_height = 400

    for period in range(1, 4):
        image_bytes = generate_jupiter_image(period, canvas_width, canvas_height)

        # Save the image to a file
        output_path = pathlib.Path(__file__).parent / "output" / f"jupiter_image_{period}.png"
