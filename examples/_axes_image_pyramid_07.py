...
class PyramidConfig:
    ...
    new_subsampled_files: bool = True

    image_interpolation: ImageInterpolation = ImageInterpolation.LINEAR
    # image_interpolation: ImageInterpolation = ImageInterpolation.NEAREST
    """Interpolation method for the image visual. 
    
    It can be either NEAREST or LINEAR. NEAREST is faster but may look worse when zooming in, while LINEAR is slower but looks better when zooming in."""
    ...
    channel_count: int = 30 if new_subsampled_files else 384
    ...
    # max_zoomout: int = 11
    max_zoomout: int = 9 if new_subsampled_files else 11
    ...
    file_zoom0_size_bytes: int = 8_435_340 if new_subsampled_files else 10_797_276_672
    """File size in bytes."""
...
class PyramidTextureHelper:
    ...
    def file_level_path(zoomout_level: int) -> pathlib.Path:
        ...
        if use_local_copy:
            ...
            if PyramidConfig.new_subsampled_files:
                file_path = pathlib.Path(__file__).parent / ".." / "tmp" / "pyramid" / "pyramid_small" / f"res_{zoomout_level:02}.bin"
            else:
                file_path = pathlib.Path(__file__).parent / ".." / "tmp" / "pyramid" / "pyramid" / f"res_{zoomout_level:02}.bin"
        else:
            # Get file path for the zoomout level - when downloading the file, directly from https://github.com/datoviz/data
            file_path = AssetDownloader.download_data(f"textures/pyramid_small/res_{zoomout_level:02}.bin")
        return file_path
    ...
    def file_level_memmap(zoomout_level: int) -> np.memmap:
        ...
        """
        # get file path for the zoomout level
        file_path = PyramidTextureHelper.file_level_path(zoomout_level)
    ...
    def texture_load(zoomout_level: int, image_x_min_dunit: float, image_x_max_dunit: float) -> Texture:
        ...
        """
        # Get the file sliced numpy for the zoomout level and image x extent
        file_sliced_numpy = PyramidTextureHelper.file_sliced_numpy(zoomout_level, image_x_min_dunit, image_x_max_dunit)

        # scale file_sliced_numpy to [0, 255] uint8
        array_clipped_numpy = np.clip(file_sliced_numpy, PyramidConfig.value_min, PyramidConfig.value_max)
        # Normalize to [0, 1]
        array_normalized_numpy = (array_clipped_numpy - PyramidConfig.value_min) / (PyramidConfig.value_max - PyramidConfig.value_min)
        # Scale to [0, 255] and convert to uint8
        array_scaled_numpy = (array_normalized_numpy * 255.0).astype(np.uint8)

        # Load pyramid data from memmap file
        image_width = file_sliced_numpy.shape[0]
        image_height = file_sliced_numpy.shape[1]
    ...
    def get_desired_zoomout_level(axes_display: AxesDisplay) -> int:
        ...
        # logger.debug(f"desired_zoomout_level : {desired_zoomout_level}")
...
class PyramidImageHelper:
    ...
    def image_build(viewport: Viewport, axes_transform_numpy: np.ndarray) -> RenderItem:
        ...
        image = Image(texture, position_buffer, image_extent_dunit, PyramidConfig.image_interpolation)
    ...
    def image_should_be_updated(axes_display: AxesDisplay, image_visual: Image) -> bool:
        ...
        if desired_zoomout_level != PyramidParams.current_zoomout_level:
            should_update = True
            logger.debug(f"update due to zooming - current_zoomout_level: {PyramidParams.current_zoomout_level} desired_zoomout_level: {desired_zoomout_level}")
...
class PyramidPanZoomHelper:
    ...
    def create(axes_display: AxesDisplay, renderer_base: RendererBase) -> AxesPanZoom:
        ...
        # Create the pan/zoom handler itself
        ...
        # Return the pan/zoom handler
        return axes_pan_zoom

    @staticmethod
    def init(axes_display: AxesDisplay, animator: AnimatorBase, onRender: typing.Callable[[], None]) -> None:
        """Initialize the pan/zoom handler by subscribing to axes limits change and animator events.

        Render at most at 60 FPS to avoid blinking during interaction, but always render on axes limits change.

        Args:
            axes_display (AxesDisplay): AxesDisplay containing the image.
            animator (AnimatorBase): Animator to subscribe to for rendering.
...
def main():
    ...
    # Create a canvas
    canvas = Canvas(width=800, height=800, dpi=127, background_color=Constants.Color.white)
    # Create renderer
    ...
    inner_viewport = Viewport(
        x=viewport_margin_x,
        y=viewport_margin_y,
        width=int(canvas.get_width() - 2.0 * viewport_margin_x),
        height=int(canvas.get_height() - 2.0 * viewport_margin_y),
        background_color=Constants.Color.transparent,
    )
    ...
    def render_axes(force_update: bool = False) -> None:
        ...
        It also updates the image visual if needed based on the axes limits change.

        Args:
            force_update (bool): If True, force update the image visual even if not needed. Default is False.
        """
