...
def build_scene() -> ReviewScene:
    ...
    return ReviewScene(
        ...
        visuals=(
            ...
            TextVisual(id="visual:labels", texts=("alpha", "beta", "gamma"), positions=positions + np.array([[0.06, 0.06]], dtype=np.float32), coordinate_space=CoordinateSpace.NDC, rgba=np.repeat(np.array([[20, 20, 20, 255]], dtype=np.uint8), 3, axis=0), font_size_px=18.0, anchor_x=TextAnchorX.LEFT, anchor_y=TextAnchorY.BOTTOM, z_order=2),
        ),
