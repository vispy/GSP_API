...
class Viewport:
    def __init__(self, canvas: Canvas, x: int, y: int, width: int, height: int):
        ...
        self.gsp_view_matrices: GspBuffer = Bufferx.mat4_identity()
        self.gsp_proj_matrices: GspBuffer = Bufferx.mat4_identity()

    def add(self, visual: Pixels, view_matrix: np.ndarray, proj_matrix: np.ndarray):
        self.visuals.append(visual)
        self.gsp_view_matrices = Bufferx.from_numpy(view_matrix, GspBufferType.mat4)
        self.gsp_proj_matrices = Bufferx.from_numpy(proj_matrix, GspBufferType.mat4)
