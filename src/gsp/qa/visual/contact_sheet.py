...
def write_contact_sheets(
    ...
    *,
    ...
    paths.append(_write_all_cases_sheet(out_dir, case_reports, backends, suite=suite))
    return tuple(paths)


def _write_case_sheet(
    out_dir: Path, case_report: Mapping[str, object], backends: tuple[str, ...]
) -> Path:
    case_id = str(case_report["case_id"])
    slug = case_id.replace("/", "_")
    path = out_dir / "contact_sheets" / f"{slug}.png"
    fig, axes = plt.subplots(
        1, len(backends), figsize=(4 * len(backends), 4), squeeze=False
    )
    try:
...
def _write_all_cases_sheet(
    ...
    *,
    ...
    path = out_dir / "contact_sheets" / f"{suite}_all_cases.png"
    fig, axes = plt.subplots(
        len(case_reports),
        len(backends),
        figsize=(4 * len(backends), 3.2 * len(case_reports)),
        squeeze=False,
    )
    try:
...
def _draw_tile(axis: Axes, case_report: Mapping[str, object], backend: str) -> None:
    ...
    if (
        ...
        and isinstance(entry.get("artifact_path"), str)
    ):
