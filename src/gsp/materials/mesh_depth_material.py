...
class MeshDepthMaterial(MeshMaterial):
    """A material that colors each face by its depth (near = white, far = black)."""

    def __init__(self, edge_colors: TransBuf, edge_widths: TransBuf, face_sorting: bool, face_culling: Constants.FaceCulling):
        """Initialize a MeshDepthMaterial instance.

        Face colors are computed at render time from NDC z (mean over the triangle):
        NDC z in [-1, +1] is mapped to grayscale in [1, 0] (near = white, far = black).
